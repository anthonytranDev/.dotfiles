...
function count_elements --no-scope-shadowing
    count (string split ',' (string join '' ($argv)))
end
...
function settings
    set -l ITEMS string split ', ' (defaults domains)

    if test $argv[1] = --list || test $argv[1] = -l
        $ITEMS
    end

    if test $argv[1] = --find || test $argv[1] = -f
        $ITEMS | grep --color='always' $argv[2]
        echo
        echo Searched items: (count ($ITEMS | grep --color='always' $argv[2]))
    end

    if test $argv[1] = --total
